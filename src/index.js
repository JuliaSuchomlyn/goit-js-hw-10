...
import { createMarkupCountryList, createMarkupCountryInfo } from './js/createMarkup';
...
function onCountryInput() {
  ...
  fetchCountries(name)
    .then(country => {
      ...
      if (country.length === 1) {
        refs.info.insertAdjacentHTML('beforeend', createMarkupCountryInfo(country));
      } else if (country.length >= 10) {
        ifTooManyMatchesAlert();
      } else {
        refs.list.insertAdjacentHTML('beforeend', createMarkupCountryList(country));
      }
    })
